...
//    }
    ...
    func test_preCatalina()
    {
        let afterFirstUnlockOnlyQuery: [CFString : Any] = [
            kSecClass : kSecClassGenericPassword,
            kSecAttrService : "nick_wanted_a_service",
            kSecAttrAccount : "nick_wanted_to_test_this",
            kSecReturnAttributes : true,
            kSecAttrAccessible : Accessibility.afterFirstUnlock.secAccessibilityAttribute,
        ]
        var whenUnlockedQuery = afterFirstUnlockOnlyQuery
        whenUnlockedQuery[kSecAttrAccessible] = Accessibility.whenUnlocked.secAccessibilityAttribute
        print(SecItemDelete(afterFirstUnlockOnlyQuery as CFDictionary))
        SecItemDelete(whenUnlockedQuery as CFDictionary)

        var result: AnyObject? = nil
        var status = SecItemAdd(afterFirstUnlockOnlyQuery as CFDictionary, &result)
        XCTAssertEqual(status, errSecSuccess)
        status = SecItemCopyMatching(whenUnlockedQuery as CFDictionary, &result)
        XCTAssertEqual(status, errSecItemNotFound)

        SecItemDelete(afterFirstUnlockOnlyQuery as CFDictionary)
        SecItemDelete(whenUnlockedQuery as CFDictionary)
    }

    func test_nleg_preCatalina()
      {
        let afterFirstUnlockOnlyQuery: [CFString : Any] = [
          kSecClass : kSecClassGenericPassword,
          kSecAttrService : "nick_wanted_a_service",
